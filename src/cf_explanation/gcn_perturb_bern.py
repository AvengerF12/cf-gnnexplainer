...
# This implementation of the explainer uses the Bernoulli approach described in Srinivas et al.
# while keeping the original paper's approach to perturbation (Adj * P)
class GCNSyntheticPerturbBern(nn.Module):
    """
    3-layer GCN used in GNN Explainer synthetic tasks
    """
    def __init__(self, nfeat, nhid, nout, nclass, adj, dropout, beta, edge_del=False,
                 edge_add=False):
        super(GCNSyntheticPerturbBern, self).__init__()
        ...
        if self.edge_add:
            ...
            self.P_vec = Parameter(torch.FloatTensor(adj_vec))
        else:
            self.P_vec = Parameter(torch.FloatTensor(torch.ones(self.P_vec_size)))

        # The bernoulli approach should be robust wrt init
