...
def gridsearch():
    ...
    for i, combo in enumerate(combo_list):
        ...
        # Explanations too big
        if combo_dict["edge_add"] and not combo_dict["delta"] and not combo_dict["bernoulli"]\
                and dataset_id == "syn1":
            continue

        if combo_dict["cem_mode"] is None and not combo_dict["edge_add"] \
                and not combo_dict["edge_del"]:
            continue
